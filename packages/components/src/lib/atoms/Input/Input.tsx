 ...
 */
export interface InputProps extends FormElementProps, VoidElementTagProps {
  /**
   ...
   * text for the image, displaying the value of the attribute if the image src
   * is missing or otherwise fails to load.
   */
  alt?: string
  ...
  /**
   * The URL to which to submit the form's data; overrides the form's `action`
   * attribute, if any.
   */
  formAction?: string
...
/**
 * Renders an `<input>` element with the class `mb-ada-input`.
 *
 * **https://developer.mozilla.org/docs/Web/HTML/Element/input**
 */
export const Input: FC<InputProps> = ({ innerProps, innerRef, ...rest }) => {
  const mutatedProps = useMutatedProps(
    omit(merge(rest, innerProps), ['options', 'selectProps', 'theme']),
    'mb-ada-input'
  )

  return <input {...(mutatedProps as ReactInputProps)} ref={innerRef} />
}
